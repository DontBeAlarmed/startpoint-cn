...
function getDefaults(
    ...
    return {
        playerHistoryId: 1,
        // 官方履历背景转换表尚未进入 Content Snapshot；1 对应客户端 DummyRemote 的首项语义。
        backgroundCardId: 1,
...
function serializeTopics(topicVisibility: Record<string, boolean>) {
    return Object.fromEntries(Object.entries(topicVisibility).map(([topicId, visible]) => [
        topicId,
        { is_visible: visible, value_list: {} },
    ]))
}
...
const routes = async (fastify: FastifyInstance) => {
    fastify.post("/index", async (request, reply) => {
        ...
        const settings = getPlayerHistorySettingsSync(
            resolved.playerId,
            getDefaults(resolved.playerId, resolved.player),
        )
        reply.header("content-type", "application/x-msgpack")
        return reply.status(200).send({
            data_headers: generateDataHeaders({ viewer_id: resolved.viewerId }),
            data: {
                player_history_id: settings.playerHistoryId,
                background_card_id: settings.backgroundCardId,
                degree_id: settings.degreeId,
                favorite_character: {
                    character_ids: settings.characterIds,
                    unison_character_ids: settings.unisonCharacterIds,
                },
                player_history_topic_list: serializeTopics(settings.topicVisibility),
            },
        })
    })
