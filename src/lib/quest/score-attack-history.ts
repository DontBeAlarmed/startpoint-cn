...
export interface BuildScoreAttackBattleHistoryInput {
...
function requiredNonNegativeNumber(value: unknown, field: string): number {
    if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
        throw new Error(`Score attack history ${field} must be a finite non-negative number`)
    }
    return value
}
...
function optionalPositiveInteger(value: unknown, field: string): number | null {
    ...
    if (!Number.isSafeInteger(value) || Number(value) <= 0) {
        throw new Error(`Score attack history ${field} must be a positive integer or null`)
    }
    return Number(value)
}
...
function optionalNonNegativeInteger(value: unknown, field: string): number | null {
    if (value === undefined || value === null) return null
    if (!Number.isSafeInteger(value) || Number(value) < 0) {
        throw new Error(`Score attack history ${field} must be a non-negative integer or null`)
    }
    return Number(value)
}
...
function sumMemberDamage(
    ...
    for (const [zoneIndex, zone] of zones.entries()) {
        ...
        if (!Number.isFinite(total)) throw new Error("Score attack history member damage overflow")
        found = true
    }
...
export function buildScoreAttackBattleHistoryRecord(
    ...
    const elapsedTimeMs = requiredNonNegativeNumber(input.elapsedTimeMs, "elapsed_time_ms")
    const score = input.score === null
        ? null
        : requiredNonNegativeNumber(input.score, "score")
    const clearRank = optionalNonNegativeInteger(input.clearRank, "clear_rank")
    const zones = input.statistics.zones
    if (!Array.isArray(zones) || zones.length === 0) {
        throw new Error("Score attack history statistics.zones must not be empty")
    }
    let totalDamage = 0
    for (const [index, zone] of zones.entries()) {
        ...
        if (!Number.isFinite(totalDamage)) throw new Error("Score attack history total damage overflow")
    }
    ...
    const equipmentSnapshots = equipments.map((equipmentId, index) => {
        ...
        if (!equipment) {
            throw new Error(`Score attack history equipment${index + 1}_id is not owned`)
        }
        return equipment
    })
    ...
    return {
        ...
        character_1_total_damage: sumMemberDamage(zones, 0),
        character_2_total_damage: sumMemberDamage(zones, 1),
        character_3_total_damage: sumMemberDamage(zones, 2),
