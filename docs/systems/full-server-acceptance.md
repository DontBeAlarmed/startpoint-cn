...
2026-08-22 的三轮正式自动验收已通过。三轮均满足以下稳定规模和结构门禁，且已包含 runtime descendants 清理修复：
...
## Gate 38：2026-08-25 最终组合复核

本次复核基于当前 `dev` 代码重新运行正式模式三轮，未重写既有参考值，也未修改生产协议或测试专用边界。三轮均满足正式 profile：非多人使用 1000 份隔离存档、600 个活跃身份和并发 `10/25/50/100`；多人使用 120 个身份、60 个双人房间和并发 `5/10/20`，其中 30 个 Host-owned、30 个 Client-owned。

| 门禁 | 结果 |
|---|---|
| 非多人每档 600 请求完成 | 通过，错误 0、行为签名稳定、回滚通过 |
| 多人每档 60 房间/120 身份完成 | 通过，Host-owned 与 Client-owned 均为 30/30 |
| HTTP 共存 auth/load/mission | 通过，所有共存请求完成且错误 0 |
| 重复 finish 与 active quest | 通过，重复 finish 全部拒绝，结束后 active quest 为 0 |
| 房间、peer、进程、端口和临时目录清理 | 通过，三轮均归零 |
| 同机延迟回归 | 通过，非多人比值 `1.091331`，多人比值 `1.071596`，均低于 `1.2` |

本次 Gate 只证明优化后的模块组合没有出现结构、行为、事务或资源清理回归，不证明多人 finish 的 91 条语句已经达到理论最小值。多人子进程仍没有测试专用 SQL/事件循环 IPC；多人 SQL 结论继续以快照和结算专项 runner 的结构上界为准。
