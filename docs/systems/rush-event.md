...
`eventId - 10` 没有官方服务端抓包证明，属于为保证常驻活动可玩性而保留的**推测性兼容实现**，不能描述为 CDN 原始行为。官方 Content Snapshot 与商品 catalog 只包含官方数据，兼容内容在查询期组合（`src/lib/shop/rush-final-operation-override.ts`，`PRIVATE_OVERRIDE` 来源标记）；文件夹奖励、商店列表和常驻购买期三个维度由同一个后台开关整体控制。读取时始终优先使用常驻活动自身的非空数据；空商品对象和空文件夹奖励数组表示该部分仍未补全，开启时继续回退。关闭开关后完全回到官方末期空奖励、空商品行为。未来应通过 CDN 补丁层补齐常驻活动商品和奖励，或根据官方响应替换本兼容层。
