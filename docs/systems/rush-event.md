# 狂热激战（Rush Event）
> 状态：关卡流程已实现；常驻批次兼容由后台策略开关控制（默认开启），待客户端验收

本文记录狂热激战的关卡、商店和奖励边界。国服 CDN `v1.4.54` 包含原始活动和常驻活动两组数据；常驻活动缺少独立兑换数据，服务端保留一层可剥离、后台可关闭的私服兼容。
...
| 常驻活动批次 | `700011-700017` | 代币为 `(None)`、文件夹奖励为空、商品为 0 件 | 由 `rush700011To700017CompatibilityEnabled`（默认开启）控制是否映射到 `eventId - 10` 的完整文件夹通关奖励和商品 |

`eventId - 10` 没有官方服务端抓包证明，属于为保证常驻活动可玩性而保留的**推测性兼容实现**，不能描述为 CDN 原始行为。官方 Content Snapshot 与商品 catalog 只包含官方数据，兼容内容在查询期组合（`src/lib/rush-final-operation-override.ts`，`PRIVATE_OVERRIDE` 来源标记）；文件夹奖励、商店列表和常驻购买期三个维度由同一个后台开关整体控制。读取时始终优先使用常驻活动自身的非空数据；目标活动及 folder 行明确存在且奖励为空数组时才允许回退，目标行缺失或畸形时拒绝结算。关闭开关后完全回到官方末期空奖励、空商品行为。未来应通过 CDN 补丁层补齐常驻活动商品和奖励，或根据官方响应替换本兼容层。
...
兼容商品保留原始开放期，并在开关开启时额外增加常驻活动 CDN 给出的开放期 `2025-06-26 12:00:00` 至 `2025-08-14 23:59:59`（UTC+8）。列表和购买使用同一个多时间段判断；两个时间段之间不会保持开放。附加开放期采用追加并去重，不覆盖商品已有时段；解析时会拒绝不存在的日历日期和越界时间。开关关闭时不附加该时段，常驻期直购按原始开放期以 `2053` 拒绝。
...
- `tools/rush_final_operation_override.test.cjs`：兼容范围限定（`700011-700017` 之外不含 `700099` 与 Hard Multi `1001-1006`）、文件夹奖励官方/私服来源区分、开关关闭回官方空数据、官方 catalog 不被污染、非法官方行拒绝而非静默回退。
