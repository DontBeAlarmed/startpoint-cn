...
兼容商品保留原始开放期，并额外增加常驻活动 CDN 给出的开放期 `2025-06-26 12:00:00` 至 `2025-08-14 23:59:59`（JST）。列表和购买使用同一个多时间段判断；两个时间段之间不会保持开放。附加开放期采用追加并去重，不覆盖商品已有时段；解析时会拒绝不存在的日历日期和越界时间。
...
- `tools/rush_event_shop.test.cjs`：主数据数量、兼容映射、空/非空精确数据覆盖、文件夹通关奖励（代币及配套素材）、JST 边界与非法日期、数量校验、库存/余额和事务回滚。
