...
| 称号（category 5） | 1288 | 1274，约 98.9% | 新增 84 条指定 Boss/Advent 累计通关、46 条权威战斗统计、珍品商店 Mana 与装备觉醒 6 条、角色等级 2 条；14 条权威事实阻塞已精确归档并继续 fallback |
...
称号自动计算覆盖现为 `1274/1288`，约 `98.9%`；其余 14 条已按精确 ID 归档为权威事实阻塞。84 条指定 Boss/Advent 累计通关从成功 finish 增长；46 条战斗统计按 battle kind 读取 zone 累计和历史最大值；珍品商店 Mana 与装备觉醒 6 条在业务事务内增长；角色等级 2 条读取 Lv80/Lv100 EXP 阈值。延期 ID 与重新开放前置见[任务系统后续路线图](mission-roadmap.md)，实现边界见[待审阅实现记录](mission-implementation-assumptions.md)。
