...
| 称号（category 5） | 1288 | 1268，约 98.4% | 新增 84 条指定 Boss/Advent 累计通关、46 条权威战斗统计、珍品商店 Mana 与装备觉醒 6 条；unsupported 20 条继续 fallback |
...
称号自动计算覆盖现为 `1268/1288`，约 `98.4%`，unsupported 为 20 条。84 条指定 Boss/Advent 累计通关从成功 finish 增长；46 条战斗统计按 battle kind 读取 zone 累计和历史最大值；珍品商店 Mana 与装备觉醒 6 条在业务事务内增长。MVP 与魂珠设置继续 fallback，完整边界见[待审阅实现记录](mission-implementation-assumptions.md)。
