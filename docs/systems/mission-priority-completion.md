...
| 称号（category 5） | 1288 | 1262，约 98.0% | 新增 84 条指定 Boss/Advent 累计通关和 46 条权威战斗统计，以及既有角色、装备、关卡、库存与养成条件族；unsupported 26 条继续持久化 fallback，其中 MVP 因无法由服务端复算而保留 |
...
称号自动计算覆盖现为 `1262/1288`，约 `98.0%`，unsupported 为 26 条。84 条指定 Boss/Advent 累计通关从成功 finish 直接增加匹配任务进度；46 条战斗统计按主数据 battle kind 读取 zone 累计和历史最大值。所有事实与旧进度取最大值，非法字段 fail closed。MVP 因服务端缺少全员分数无法复算，继续 fallback；完整边界见[待审阅实现记录](mission-implementation-assumptions.md)。
