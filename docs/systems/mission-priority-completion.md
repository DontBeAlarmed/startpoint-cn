...
|---|---:|---:|---|
| 角色觉醒（category 9） | 144 | 141 条权威计算，3 条 fail closed；55 条通用角色通关使用固定白名单 | 核心流程已通过客户端；3 条语义待权威证据，全部条件待逐条验收 |
...
| 称号（category 5） | 1288 | 1274，约 98.9% | 新增 84 条指定 Boss/Advent 累计通关、46 条权威战斗统计、珍品商店 Mana 与装备觉醒 6 条、角色等级 2 条；unsupported 14 条继续 fallback |
...
`1610022` 与 `2610072` 的主数据 pattern type 为 `95`，不是普通队长通关。主数据可以确认
`statistics=17`、指定队长和 `battle_kind=3`，但现有 CN 1.8.1 反编译不能把 statistics 17 权威映射到
`encoffin_count` 或其他 finish 字段。

当前两条任务 fail closed：不根据文案、`continue_count`、最终存活角色数或相邻统计编号猜测，不新增进度；
旧 category 9 持久进度继续作为下限。获得权威字段映射后再增加正反例和事务测试。
...
144 条定义；`1610022/2610072` 与种族 selector 任务 `2310012` 明确标为 fail closed。
...
称号自动计算覆盖现为 `1274/1288`，约 `98.9%`，unsupported 为 14 条。84 条指定 Boss/Advent 累计通关从成功 finish 增长；46 条战斗统计按 battle kind 读取 zone 累计和历史最大值；珍品商店 Mana 与装备觉醒 6 条在业务事务内增长；角色等级 2 条读取官方 Lv80/Lv100 EXP 阈值。MVP 与魂珠设置继续 fallback，完整边界见[待审阅实现记录](mission-implementation-assumptions.md)。
...
- 角色觉醒：144 条静态唯一分区、55 条通用 schema、四条 QuestRange、强化弹射/连击/三角色同场、指定队长+关卡/限时、失败与重复 finish、单人/协力边界及不回退；statistics 17 与种族 selector 固定为待证据。
