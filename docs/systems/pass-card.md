...
游戏内不再通过支付 SDK 购买高阶资格：`/payment/item_list` 永远返回空商品列表，因此 Pass 页面不会打开高阶修行礼包的支付弹窗，也不会调用雷霆 SDK 支付。现有 `/payment/start` 与 `/payment/finish` 仅保留协议兼容代码；若被外部直接调用，`/payment/finish` 仍可按服务器时间将 `com.leiting.wf.pass_card` 的 `is_buy` 持久化为 `true`，但不发放星导石，也不代表真实支付成功。后续其他授予方式需单独定义，不由游戏内支付流程隐式触发。付费轨奖励仍需通过 `receive_all` 单独领取。
