...
游戏内不再通过支付 SDK 购买高阶资格：`/payment/item_list` 永远返回空商品列表，因此 Pass 页面不会打开高阶修行礼包的支付弹窗，也不会调用雷霆 SDK 支付。当前开放活动对所有存档默认授予高阶资格：新存档初始化时写入 `is_buy=1`，旧存档请求 `get_pass_card` 时幂等修正为 `true`；点数、登录基线和领取记录不变。现有 `/payment/start` 与 `/payment/finish` 仅保留私服协议兼容代码。`/payment/finish` 不验证收据、签名或支付平台回调；若外部直接提交的商品 ID 存在于 `assets/payment_products.json`，它会按配置增加付费/免费星导石，`com.leiting.wf.pass_card` 还会设置当前开放活动的购买资格。因此“未接入真实支付”不等于该路由不会产生游戏内资产变化，部署时必须阻止不可信调用。高阶奖励仍通过 `receive_all` 按原有记录领取，不在授予资格时直接发放。
