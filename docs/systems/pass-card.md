...
私服兼容的高阶资格已接入 `/payment/finish`：当商品为 `com.leiting.wf.pass_card` 时，服务端只将服务器时间下当前开放期的 `is_buy` 持久化为 `true`，不发放星导石，也不代表真实支付成功。重复请求是幂等的；普通商品不受影响。付费轨奖励仍需通过 `receive_all` 单独领取。
