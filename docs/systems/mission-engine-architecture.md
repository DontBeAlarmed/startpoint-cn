...
- 普通分类任务 Category 1～10 的阶段完成即自动发奖；`mission_info` 表示本次请求已经成功写入领取阶段并发放奖励，不是“等待玩家手动领取”。
- 事实生产者必须在事实所属业务事务内定向结算本次候选，并把任务、物品、角色、装备、称号和玩家余额合并进同一次响应。当前覆盖登录、客户端白名单进度上报、角色故事、战斗 finish、装备觉醒、魂珠装配、珍奇品兑换、Raid summary、Raid SET 保存和角色投票。
- Active Mission 保持独立语义：事实结算只推进到 `received=false`，玩家通过 `/active_mission/receive` 手动领取奖励。
- Pass 任务事实按普通分类任务自动结算并增加 Pass 点数；Pass 等级的免费/高阶轨奖励仍由 `receive_all` 手动领取。
- `/mission/get_mission_progress` 保留开放任务补结算和旧存档修复能力，并负责生成任务页聚合响应；它不是正常业务事实的首个发奖时点。重复调用必须幂等，不能重复发奖。
- Category 9 的持久觉醒解锁与普通奖励领取继续遵守觉醒专用页面时序，不因普通分类任务即时结算而改变。
