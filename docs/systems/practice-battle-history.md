...
`players_practice_battle_history` 登记在存档 V2 的 `events` 领域，导出、恢复和克隆均覆盖；克隆时自增 `id` 重新分配。该表在 schema 13 引入；当前 Server Bundle 目标数据版本已随账号管理与继承码字段升级为 17。
...
- finish 总事务成功与回滚；
- schema 13 引入、schema 17 当前兼容、存档 V2 和 Server Bundle 数据版本契约。
