...
3. 聚合同道具/装备/货币后调用现有奖励 helper；角色、称号和 `user_info` 变化进入通用响应。
4. 使用 B/A/S/SS 阈值计算 C=1、B=2、A=3、S=4、SS=5。
5. 在一个 SQLite 事务中发奖、写入最高分/最高评级/最佳耗时并删除持久化 active quest；提交后再清理内存 active quest。
6. 同分或低分仍可更新更短耗时，但不会重复发放分数档奖励。

CN 当前奖励槽全部是道具类型；实现同时支持装备、角色、星导石、玛纳、经验池和称号。主数据若未来出现尚未实现的通行证点数类型，会明确失败并保留 active quest，不会静默错发。
