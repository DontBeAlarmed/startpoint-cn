...
- 登录奖励活动组选择使用全局虚拟时间，并与 CDN 的活动开放期一致。
- 登录奖励领取资格使用真实业务时间，并以 `DAILY_RESET_HOUR`（默认凌晨 5:00）划分真实业务日。
- 因此调整虚拟时间不会提前发放或阻止下一档登录奖励；虚拟时间只影响当前哪一组奖励处于开放状态。
...
2. 若当前真实业务日不晚于上次已发奖真实业务日，返回空奖励状态；服务器时间回拨不会重复发奖。
...
`players_login_bonus_progress` 按 `(player_id, group_id)` 保存每组的最后发放 index、虚拟业务日、真实业务日、领取时间和展示确认时间。玩家删除时由外键级联清理；时间导入和虚拟时间调整不删除已发放状态。结算还会比较玩家所有组的最大已发放真实业务日，防止回拨后重新领取更早日期的未领取奖励。

旧存档没有真实业务日字段。第一次登录奖励结算时只建立当前真实业务日基线，不补发奖励；从下一个真实业务日开始按新规则继续推进。

性能边界：一次登录奖励结算只批量读取该玩家的全部组进度，随后在内存中按 `group_id` 复用；不会随着同时有效的奖励组数量逐组查询同一张表。奖励组目录来自已加载的 Content Snapshot，结算不重新读取 CDN 文件。

## 时间回拨与边界

- 登录奖励的活动组属于虚拟运营日历：向前调整可能进入新的 CDN 活动期，向后调整不会重发已确认奖励。
- 登录奖励的每日领取属于真实运营日历：只有真实时间跨过凌晨 5:00 才会推进下一档；调整虚拟时间不会改变领取日。
