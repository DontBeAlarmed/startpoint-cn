...
国服客户端没有独立的“自动连战体力不足” start 响应类型；客户端已有的非致命返回到配队页通道使用 `data_headers.result_code=4050`。服务端只在自动连战且确定为体力不足时以 HTTP 200 返回该结果码，普通手动挑战仍返回入场错误。这个复用官方客户端“超出开放期”错误码的做法是兼容性推断，可能导致客户端提示文案按其内置逻辑显示，待客户端验收确认。
