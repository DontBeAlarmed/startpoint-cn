...
- 角色等级称号 `3010/3020` 分别按各稀有度现有 Lv80/Lv100 上限 EXP 阈值计算当前可证明的最高等级，并与持久化进度取最大值。Gate 29a 已从普通 CDN 与客户端 bundled master 合并出 1～5 星的完整精确曲线；`3000` 在专门接入 Content snapshot 等级表并补齐 mission 回归前仍保持 fallback，本阶段不顺带改变生产计算器。
