...
- category 1 使用终身累计事实：SS/S/A/B 对应 `clear_rank` 5/4/3/2；单人、协力、房主和成员通关均为成功次数；
  玩家等级、累计登录、冲刺、强化弹射和最高连击读取各自权威字段。普通累计登录任务包含 `total_login` 和
  `special_total_login_2anv` 两个主数据 pattern；后者的客户端文案为“【特别】累计登陆天数 ::x_count::”，
  阈值由 `mission_regular_reward` 的 18 个阶段提供（2～10、20、30、40、50、100、150、200、250、300）。
  `special_total_login_2anv` 的 `2023-08-31 12:00:00` 历史日期字段是周年追加标记，不作为当前运行时开放窗口；
  它与普通累计登录一样读取账号累计登录天数，并在 `/load` 的登录事实边界自动完成和发奖。
...
- 当前存档导入导出格式尚未包含 `players_collected_items`。替换存档后不能从当前库存反推历史累计值；这是已知边界，
  本轮按需求不扩展完整数据库导入导出。
- category 5 共 1288 条。当前自动计算 1282 条有权威事实的任务：玩家等级 8 条、角色等级 3 条、持有角色数 3 条、
...
- 当前称号覆盖为 `1282/1288`，约 `99.5%`；unsupported 为 6 条。14 条指定 Boss 超级难度称号按 `quest_rank` 的敌人等级区间解析，包含不按 ID 后缀排列的废墟魔像和缺少中级难度的大蛇；84 条指定 Boss/Advent 累计通关称号按主数据 selector 和官方关卡表精确匹配；46 条称号读取成功 finish 的分 battle kind zone 累计、单场最大战斗统计；珍品商店 Mana、装备觉醒和魂珠装配从业务事务增长；4 条展示/点击称号读取官方客户端静默请求；MVP 使用官方客户端 `statistics.is_mvp` 字段。
- 6 条 fallback 已由 `coverage-audit.ts` 按精确 ID 固定为延期集合：`25000/25010/25020`；`70004/70005/70006`。它们依次对应 Attention 和协力新手。覆盖测试锁定 ID 与机器可读原因；新增事实族必须同时移出对应延期 ID，不能只改文档数字。
- 其余已实现称号只读取对应的持久化角色、装备、关卡、库存、商店或成功结算事实，并与旧进度取最大值。未知主数据、非法客户端统计和缺少权威生产者的条件继续 fallback，不根据中文描述或相邻任务推算。
- 角色等级称号 `3000/3010/3020` 分别按各稀有度现有 Lv60/Lv80/Lv100 EXP 阈值计算当前可证明的最高等级，并与持久化进度取最大值。Gate 29a 已从普通 CDN 与客户端 bundled master 合并出 1～5 星的完整精确曲线，三条角色等级称号现进入同一权威计算路径。
...
- 18 条规则逐 ID 校验官方 pattern、严格十进制 pattern type/奖励 target、可选 QuestRange selector、保留空字段和精确 `YYYY-MM-DD HH:mm:ss` UTC+8 日历开放期；空串、首尾空白、溢出整数、不存在日期和任一主数据漂移均 fail closed。
  ...
  官方表派生索引按启动后冻结的 Content repository 对象缓存，不支持也不引入热更新。evaluationTime 下 15 条任务全部关闭时不会形成候选，因此不会构造 Event Session category context，也不会触发角色、玛纳板、装备、物品或 party fact loader。
...
`src/lib/mission/coverage-audit.ts` 是覆盖数字和剩余 ID 的唯一机器清单。`tools/mission_coverage_audit.test.cjs` 锁定 category 3 `2485/2512`、Degree `1282/1288`、觉醒条件族 `144/144`（resolved 144、fail closed 0）和 Pass `248/267`，并要求分区无交集且每个 fallback/fail-closed 条目都有原因。该报告证明代码路由和事实生产者覆盖，不等价于 CN 客户端验收。

- category 4 已形成累计获得量、活动隔离、结算、发奖和 load 映射；category 5 已接入上述 1282 条权威事实。
