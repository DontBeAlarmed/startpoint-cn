...
- 14 条 fallback 已由 `coverage-audit.ts` 按精确 ID 固定为延期集合：`3000`；`8000/8010/8020`；`11080`；`25000/25010/25020`；`26000/26010/26020`；`70004/70005/70006`。它们依次对应 Lv60 角色等级、魂珠设置、主数据不存在的指定 Boss 难度、Attention、MVP 和协力新手。覆盖测试锁定 ID 与机器可读原因；新增事实族必须同时移出对应延期 ID，不能只改文档数字。
...
`src/lib/mission/coverage-audit.ts` 是覆盖数字和剩余 ID 的唯一机器清单。`tools/mission_coverage_audit.test.cjs` 锁定 category 3 `2485/2512`、Degree `1274/1288`、觉醒条件族 `144/144`（resolved 144、fail closed 0）和 Pass `229/267`，并要求分区无交集且每个 fallback/fail-closed 条目都有原因。该报告证明代码路由和事实生产者覆盖，不等价于 CN 客户端验收。
