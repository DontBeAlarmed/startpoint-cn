...
- 当前存档导入导出格式尚未包含 `players_collected_items`。替换存档后不能从当前库存反推历史累计值；这是已知边界，
  本轮按需求不扩展完整数据库导入导出。
- category 5 共 1288 条。当前自动计算 1216 条有权威服务端事实的任务：玩家等级 8 条、持有角色数 3 条、
...
- 当前称号覆盖为 `1216/1288`，约 `94.4%`；unsupported 为 72 条。84 条指定 Boss/Advent 累计通关称号按主数据 selector 和官方关卡表精确匹配；每次成功 finish 直接原子增加对应 category 5 进度，因此可从升级前已有进度继续增长，不从唯一完成行重建重复次数。
...
- category 4 已形成累计获得量、活动隔离、结算、发奖和 load 映射；category 5 已接入上述 1216 条权威事实。
