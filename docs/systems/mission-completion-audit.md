...
- 单人和多人成功结算统一通过 `recordMissionBattleFacts` 写入角色通关、队长 power flip、队伍组合、
  种族组合及全局战斗计数。失败或未达成的战斗在入口直接返回，不再增加任务事实。
...
因此，分类隔离存储、主数据列解释、客户端静默上报边界、成功战斗事实、重置基础、ActiveMission
领奖安全以及角色觉醒核心时序和已知事实错误修复已经具备，但任务模块整体仍是部分完成。下一阶段
扩展普通、每日、每周的通用自动结算，再处理活动、收集、称号与 Pass。
