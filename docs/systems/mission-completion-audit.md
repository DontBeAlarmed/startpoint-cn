...
`src/lib/mission/coverage-audit.ts` 是覆盖数字和剩余 ID 的唯一机器清单。`tools/mission_coverage_audit.test.cjs` 锁定 category 3 `1527/2512`、Degree `1274/1288`、觉醒条件族 `144/144`（其中 resolved 141、fail closed 3）和 Pass `229/267`，并要求分区无交集且每个 fallback/fail-closed 条目都有原因。该报告证明代码路由和事实生产者覆盖，不等价于 CN 客户端验收。
