...
- 230 条 `finish` 实为限时通关任务，审计计算器已按奖励表秒数和最佳毫秒记录修正，不再把一次普通通关判定为全部档位完成。
  该计算器只用于审计和后续规则迁移。
- 在类型化活动事实完成前，category 3 使用持久化 fallback：任务页不从全历史 quest progress 推算进度，
  也不进入通用自动结算和发奖。后续实现必须在业务事实发生时按开放期和关卡范围增量记录，不能回填不可靠的历史重复次数。
...
- category 3 需要类型化活动事实和结算期匹配，当前明确停用旧 map 自动结算。
