...
状态：核心优化与 Task 9 正式混合负载基线已完成；runtime descendants 清理最终门禁待确认，客户端人工验收待执行
...
Task 9 已建立正式自动基线：使用约 1000 份隔离存档、600 个活跃身份，分档运行混合登录、建房、入房、TCP 心跳和结算负载；多人覆盖 120 个身份、60 个双人房间，并按 30 个 Host-owned 与 30 个 Client-owned 分布验证。基线报告记录了请求错误、半状态、场景完成数、资源清理和 SQL/快照结构结果；正式 reference 只保留 fingerprint、profiles 和 mediansMs，不写入身份、房号、令牌、端口、路径或原始错误。48abad09 后的 runtime descendants 清理另有定向回归，最终自动门禁尚待确认。

最终三轮正式基线的非多人最大 p95 为 5778.724 / 6069.633 / 6213.359 ms，多人最大 p95 为 3139.301 / 3532.562 / 3294.842 ms；各轮均满足同机 20% 回归门槛。该结果证明自动验收范围内的组合负载与清理边界，不替代 CN 客户端、Android 壳或真实多设备人工验收。
