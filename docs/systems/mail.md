...
游戏领取分支保留 12 种历史可领取类型。其中六种标准类型映射到 RewardGrant 公共类型；六种专用类型保留邮件领域写入，不扩展 RewardGrant：
...
后台新建白名单与上述历史领取范围不同，只允许 8 种：`1` 道具、`4` 免费星导石、`5` 角色、`6` 装备、`7` 星之粒、`8` 免费玛纳、`9` 经验池、`10` 羁绊之证。后台明确禁止新建 `3` 付费星导石、`11` Boss Boost 点、`12` Boost 点和 `15` Rank 点；这些资源由玩家详情页直接调整。

历史邮件兼容只在游戏领取端生效：数据库中已有的 type 3/11/12/15 邮件仍可按原分支领取，不会因后台白名单收紧而变成不可领取。`MailType` 枚举中的 13、14、16、17 没有后台入口，也没有游戏发奖分支，领取时明确失败并保留未领取状态。
