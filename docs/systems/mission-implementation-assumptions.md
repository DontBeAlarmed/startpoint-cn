...
- `3000` 的目标为 Lv60。Gate 29a 已按客户端普通 CDN rarity 1/2 与 bundled master rarity 3/4/5 的合并语义补齐完整精确阈值，但没有改动 mission 计算器；该任务在后续专门接入 Content snapshot 等级表前继续持久化 fallback，不在本阶段顺带迁移生产行为。
