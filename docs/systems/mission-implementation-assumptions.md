...
- `3000/3010/3020` 的官方奖励目标分别为 60、80 和 100。服务端按角色官方 rarity 选择现有 `characterExpCaps`，只在累计 EXP 达到对应上限阈值时证明该档等级，并返回所有当前角色中可证明的最高等级；非法 EXP、未知角色或未知 rarity 不参与计算。
- 该计算与旧 category 5 进度取最大值，因此角色数据变化不会让已保存进度倒退。Lv60/Lv80/Lv100 阈值对五种 rarity 都存在，可精确判断这三条称号是否完成。
...
- 普通 Category 1/5/8 登录任务在基础响应构建后，于 CN MsgPack `onSend` 的短事务内定向结算并编码已合并响应；前置的每日登录计数、每日登录奖励和 Pass 登录基线仍由各自的登录前置事务负责。普通登录任务结算或编码失败只回滚本次任务进度与奖励，已提交的每日登录前置状态不回滚。
...
- 首次投票写入 `players_character_election_votes`，并在同一个事务内把 mission 2389 幂等完成到 1、自动发奖和返回 `mission_info`；事实或发奖异常会回滚投票。状态查询按玩家与 election ID 隔离，玩家数据库继续保存首次 keyword ID 和投票时刻。
...
- `character.json`、`character_quest_lookup.json`、`mana_board.json`、`main_quest.json`、`equipment_dissolve.json` 和 `item_sale.json` 的派生索引按当前冻结 Content repository 对象缓存在进程内；任一官方表行无法解析时，只有对应事实族整体 fail closed。evaluationTime 下这 15 条 current-state 任务均关闭时，不会为它们形成候选或声明 current-state facts；即使其他开放的 Event 任务仍需构造 category context，也不会因此触发角色、玛纳节点、装备、物品或 party fact loader，也不会构造 current-state 静态索引。项目不支持运行时 Content 热更新，因此不增加失效协议。
...
- 任务事实、自动发奖、队伍更新、主队伍选择和 Active Mission 队伍动作计数处于同一数据库事务；任务 SQL 失败会向外传播并整体回滚。成功响应直接返回本次 `mission_info` 与奖励变更。
...
- 正常出售装备等发放路径通过 Inventory owner 的奖励写入层同步写入库存和累计获得事实，消费锻造石不会降低称号进度；其他物品 ID 不计入。
