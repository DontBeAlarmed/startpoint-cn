...
- 首次投票写入 `players_character_election_votes`，并在同一个 `BEGIN IMMEDIATE` 事务内把 mission 2389 幂等完成到 1；任务写入异常会回滚投票。状态查询按玩家与 election ID 隔离，数据库 schema 12 继续保存首次 keyword ID 和投票时刻。
