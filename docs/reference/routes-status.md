...
| 任务 | **Partial** | 普通/每日/每周、收集、507 条权威称号、活动协力与累计物品首批规则、Pass 三分类与等级奖励、Active Mission 动态定义/可用性/安全领奖核心、角色觉醒核心时序已有实现；复杂活动谓词、Active Mission 的 96 条业务事实生产者、其余称号与 Pass 少量 pattern 尚未完成 | `src/routes/api/mission.ts`、`src/routes/api/passCard.ts`、`src/routes/api/activeMission.ts`、`src/lib/mission/` | [任务完成度审计](../systems/mission-completion-audit.md)、[Active Mission](../systems/active-mission.md)、[修行之道](../systems/pass-card.md) |
