...
| 登录奖励（非 Premium） | **Complete** | `/load` 按虚拟业务日从 CDN 选择有效的七种非 Premium 组，在一个事务内批量发奖并复用 pending 批次；`/bonus/shown` 一次确认全部 pending。Premium 类型保持空响应 | `src/routes/cn/load.ts`、`src/routes/api/bonus.ts`、`src/lib/login-bonus.ts`、`src/data/domains/login-bonus.ts` | [登录奖励](../systems/login-bonus.md) |
| 定时资源补充 | **Complete** | 后台配置全局或指定存档规则；`/load` 按真实业务日和库存下限直接发放普通道具或免费星导石，同日只记录成功发放，连续离线不补发 | `src/routes/cn/load.ts`、`src/routes/web_api/scheduled-resource.ts`、`src/lib/scheduled-resource-settlement.ts` | [定时资源补充](../systems/scheduled-resource-replenishment.md) |
...
| 玩家资料与履历 | **Partial** | `/profile/get_my_profile` 的拥有量读取真实存档、最大值读取当前 CDN；`/player_history/index` 已批量返回登录、角色、装备、任务、MVP、多人和关卡统计，并以真实时间保存主线章节、二板、百名角色和 Rank 100 的首次事实。旧存档缺失日期及尚无权威来源的主题保持 `null`，不会在打开页面时伪造历史 | `src/routes/api/profile.ts`、`src/routes/api/playerHistory.ts`、`src/lib/player-profile-stats.ts`、`src/lib/player-history-stats.ts` | [玩家资料与玩家履历](../systems/player-history.md) |
| 关注与 SNS | **Partial** | 同服 Follow 已实现（schema 28 有向边 + `lists/add/delete/delete_followed/bulk_edit/search_id`，上限读 Content config，业务错误走 result_code 1451/1452）；跨服关系、`search_twitter` 与 SNS 绑定仍延期；房间 `establisher_follow=1` 是跨服兼容投影非关系事实 | `src/routes/api/follow/`、`src/data/domains/follow.ts`、`src/multi/follow-policy.ts` | [同服 Follow 与跨服房间兼容](../systems/follow.md)、[兼容空响应与未实现路由审计](./stub-route-audit.md) |
| 抽卡 | **Partial** | 普通 draw、两类域内 exchange、Crazy 生命周期（候选/保存槽/最终选择/重登恢复）、兑换点到期转换与 shown ack 均由 Gacha owner 唯一事务协调；payment/type/数量矩阵、票券延长期、Stars/Comeback 玩家时期、批读批写与 schema26 状态已落地；网络响应丢失后的请求幂等（api_count 未证明）与部分角色动画细节仍不完整 | `src/routes/api/gacha.ts`、`src/routes/api/gacha/`、`src/lib/gacha-owner/`、`src/lib/gacha-catalog/` | [抽卡写入事务](../systems/gacha-transactions.md)、[Gacha Owner Gate](../architecture/gacha-owner-gate.md)、[卡池生成](../protocol/gacha-pool-generation.md) |
| 普通关卡 | **Partial** | 剧情 finish 已完成首通事务、剧情/城镇角色入队和幂等响应；单人战斗所有通用分类及土俑、战阵、狂热激战、练习战、无限演武专用分支共享总事务；单人 `play_continue` 按 `statistics.zones[].continue_count` 求和，使用共享 `continue_virtual_money` 费用和免费星导石优先扣款；官方负路径 result code 与服务端上限校验延期。协力 start 已按房主/成员身份事务化体力、门票与 active quest，协力奖励与进度也共享总事务；single finish 提交后不保存可重放的成功 receipt，响应阶段失败后的重试会因 active 不存在而拒绝；状态仍为 Partial，原因是多人入场和客户端分类尚未完成客户端验收，而非数据库部分提交 | `src/routes/api/singleBattleQuest.ts`、`src/routes/api/storyQuest.ts`、`src/multi/http/battle.ts`、`src/lib/quest/` | [战斗关卡结算事务](../systems/quest-finish-transactions.md)、[普通剧情结算](../systems/story-quest-settlement.md)、[关卡入场道具](../systems/quest-entry-items.md)、[体力](../systems/stamina.md) |
| 任务 | **Partial** | 普通/每日/每周、收集、1282 条权威称号、活动任务 2485 条事实、Pass 三分类与等级奖励、Active Mission 动态定义/可用性/安全领奖核心及 Contents Guide 首任务生产链、角色觉醒核心时序已有实现；拉芙 `2630023` 已按 category 18 / quest 400001104 / 贝瑞塔主位 0 / 单人成功闭合。6 条称号已按权威事实阻塞延期，category 3 救援、Pass 救援/购买和部分 Active Mission 生产者尚未实现 | `src/routes/api/mission.ts`、`src/routes/api/passCard.ts`、`src/routes/api/activeMission.ts`、`src/routes/api/contentsGuide.ts`、`src/lib/mission/` | [任务完成度审计](../systems/mission-completion-audit.md)、[任务路线图](../systems/mission-roadmap.md)、[Active Mission](../systems/active-mission.md)、[修行之道](../systems/pass-card.md) |
| 邮件 | **Partial** | 列表、后台定向发送、单领与全领已经实现；普通 Item 容量不足保留并由全领跳过，category 6 可出售 Item 可部分入包并出售差值；领取覆盖发奖、历史和邮件删除的总事务，成功响应发布 `mail_arrived` 与实际 `over_max`，但 12 种附件尚未全部客户端验收 | `src/routes/api/mail.ts`、`src/routes/web_api/mail.ts`、`src/data/domains/mail.ts` | [邮件](../systems/mail.md)、[D18b disposition](../architecture/item-overflow-disposition-gate.md) |
| 普通公告 | **Partial** | SQLite 普通公告的列表、详情、真实时间可见性、分页和后台 CRUD 已实现；系统公告、强制公告、已读和红点保持兼容空响应或延期 | `src/routes/api/news.ts`、`src/routes/web_api/news.ts`、`src/data/domains/news.ts` | [普通公告](../systems/news.md) |
| 商店与兑换 | **Partial** | Shop 的 8 类 single 与 4/7 bulk 已统一到 immutable catalog、共用 plan 和唯一 transaction owner；Mana/Special/dynamic cost、Campaign Lineup、Equipment effect、typed absolute response、Rush compatibility 及 How-to-get reverse index 已覆盖。星之粒（Star Crumb）兑换已迁移到 typed 产品/成本 catalog 与唯一事务 owner（Character/Item/Equipment 全部经 typed reward 执行并继承 D18b overflow disposition）；信赖之证（Bond Token）兑换已实现 typed 产品/成本/库存/周期 catalog、list runtime（per-player exchange_count）与唯一事务 owner，因此总分类继续标记 Partial（Special Exchange 未实现） | `src/routes/api/shop.ts`、`src/routes/api/shop/purchase-routes.ts`、`src/lib/shop/`、`src/routes/api/howToGet.ts`、`src/lib/how-to-get.ts`、`src/routes/api/exchange.ts`、`src/lib/star-crumb-exchange/`、`src/lib/bond-token-exchange/` | [商店](../systems/shop.md) |
...
| 礼包码兑换 | **Partial** | SQLite 礼包定义、精确 code、状态机、单存档领取、RewardGrant 事务、同服克隆继承和外部导入清除已实现；有效期、batch、channel、总量限制和人工重发明确延期 | `src/routes/api/gift.ts`、`src/routes/web_api/gift.ts`、`src/data/domains/gift.ts`、`src/lib/gift-code/` | [公共礼包码](../systems/gift-codes.md) |
| 教程、工具与外围兼容 | **Partial** | 首次教程已实现事务推进、重复请求和中断恢复；普通教程触发 ID 会校验、去重并批量事务写入；礼包 capability 由 active 礼包驱动；其他工具和外围路由仍可能只是维持 CN 客户端流程的空响应，必须逐注册源码确认 | `src/routes/api/tutorial.ts`、`src/routes/api/tool.ts`、`src/cn-server.ts` | [首次教程](../systems/start-tutorial.md)、[小型状态写入](../systems/small-write-boundaries.md)、[已知问题](../status/known-issues.md) |
