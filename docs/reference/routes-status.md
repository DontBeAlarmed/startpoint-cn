...
| 任务 | **Partial** | 分类存储、主数据列解释、静默上报过滤、独立战斗事实、普通/每日/每周自动结算、Active Mission 和角色觉醒核心时序已有实现；活动、收集、称号与 Pass 仍缺完整自动结算 | `src/routes/api/mission.ts`、`src/routes/api/activeMission.ts`、`src/lib/mission/` | [任务完成度审计](../systems/mission-completion-audit.md) |
