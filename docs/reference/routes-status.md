...
| 常规登录奖励 | **Complete** | `/load` 按虚拟业务日从 CDN 选择有效 `Normal` 组并事务发奖，响应中断后复用 pending 批次；`/bonus/shown` 只做幂等展示确认。限定、回归与 Premium 类型不在当前支持范围 | `src/routes/cn/load.ts`、`src/routes/api/bonus.ts`、`src/lib/login-bonus.ts`、`src/data/domains/login-bonus.ts` | [常规登录奖励](../systems/login-bonus.md) |
