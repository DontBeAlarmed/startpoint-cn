...
| 玩家履历 | **Partial** | `/player_history/index` 与 `/edit` 已按客户端协议返回并持久化收藏队伍、称号、背景和后续客户端提交的主题可见性；当前 Content Snapshot 没有官方履历期、背景和主题索引转换表，因此默认采用首期/首背景兼容值，不伪造不可追溯的历史主题统计 | `src/routes/api/playerHistory.ts`、`src/data/domains/player-history.ts` | [兼容空响应与未实现路由审计](./stub-route-audit.md) |
