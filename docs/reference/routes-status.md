...
| 角色与装备养成 | **Partial** | 角色获取、玛纳板、羁绊、分解、装备强化与保护等主流程广泛实现；玛纳节点学习、节点觉醒、信赖证领取和开板已覆盖关键多步写入事务，其他角色、装备异常回滚矩阵仍需逐路由扩展 | `src/routes/api/character.ts`、`src/routes/api/character/`、`src/routes/api/equipment.ts`、`src/routes/api/sell.ts`、`src/routes/api/exBoost.ts` | [角色养成事务](../systems/character-growth-transactions.md)、[角色分解审计](../systems/character-stack-audit.md)、[装备强化审计](../systems/equipment-upgrade-audit.md) |
