...
| 抽卡 | **Partial** | 核心角色与装备抽取、票券、兑换、权重和装备升格动画已实现；单次抽取的费用、奖励、历史、积分和任务事实共享事务，网络响应丢失后的请求幂等及部分特殊卡池、保底和角色动画仍不完整 | `src/routes/api/gacha.ts`、`src/lib/gacha*.ts` | [抽卡写入事务](../systems/gacha-transactions.md)、[扭蛋赔率修复](../systems/gacha-odds-fix.md)、[卡池生成](../protocol/gacha-pool-generation.md) |
