...
| 商店与兑换 | **Partial** | Shop 的 8 类 single 与 4/7 bulk 已统一到 immutable catalog、共用 plan 和唯一 transaction owner；Mana/Special/dynamic cost、Campaign Lineup、Equipment effect、typed absolute response、Rush compatibility 及 How-to-get reverse index 已覆盖。星之粒角色/装备兑换与其他 Exchange 仍保持独立 owner，因此总分类继续标记 Partial | `src/routes/api/shop.ts`、`src/routes/api/shop/purchase-routes.ts`、`src/lib/shop/`、`src/routes/api/howToGet.ts`、`src/lib/how-to-get.ts`、`src/routes/api/exchange.ts` | [商店](../systems/shop.md) |
