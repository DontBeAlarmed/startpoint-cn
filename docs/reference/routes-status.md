...
| 任务 | **Partial** | 普通/每日/每周、收集、507 条权威称号、活动协力与累计物品首批规则、Pass 三分类与等级奖励、Active Mission 和角色觉醒核心时序已有实现；复杂活动谓词、其余称号、Pass 少量 pattern 与购买流程尚未完成 | `src/routes/api/mission.ts`、`src/routes/api/passCard.ts`、`src/routes/api/activeMission.ts`、`src/lib/mission/` | [任务完成度审计](../systems/mission-completion-audit.md)、[修行之道](../systems/pass-card.md) |
