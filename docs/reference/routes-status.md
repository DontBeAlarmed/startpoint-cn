...
| 任务 | **Partial** | 普通/每日/每周、收集、23 条权威称号、Active Mission 和角色觉醒核心时序已有实现；活动任务旧 map 不参与进度或发奖，类型化活动事实、其余称号和 Pass 尚未完成 | `src/routes/api/mission.ts`、`src/routes/api/activeMission.ts`、`src/lib/mission/` | [任务完成度审计](../systems/mission-completion-audit.md) |
