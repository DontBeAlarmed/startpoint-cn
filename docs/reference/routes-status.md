...
| 任务 | **Partial** | 分类存储、主数据列解释、静默上报过滤、成功战斗事实、阶段、Active Mission 和角色觉醒核心时序已有实现；普通、每日、活动、收集、称号、周常与 Pass 仍缺完整自动结算 | `src/routes/api/mission.ts`、`src/routes/api/activeMission.ts`、`src/lib/mission/` | [任务完成度审计](../systems/mission-completion-audit.md) |
| 邮件 | **Partial** | 列表、单领、全领和后台定向发送已经实现；附件总事务、异常回滚及普通业务响应中的 `mail_arrived` 通知尚未统一 | `src/routes/api/mail.ts`、`src/routes/web_api/mail.ts`、`src/data/domains/mail.ts` | [邮件](../systems/mail.md) |
