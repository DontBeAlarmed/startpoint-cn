...
| 任务 | **Partial** | 普通/每日/每周、收集、23 条权威称号、939 条安全活动协力累计、Active Mission 和角色觉醒核心时序已有实现；旧活动 map 不作为计算器，复杂活动谓词、其余称号和 Pass 尚未完成 | `src/routes/api/mission.ts`、`src/routes/api/activeMission.ts`、`src/lib/mission/` | [任务完成度审计](../systems/mission-completion-audit.md) |
