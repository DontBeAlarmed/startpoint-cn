...
| 普通关卡 | **Partial** | 剧情 finish 已完成首通事务、剧情/城镇角色入队和幂等响应；战斗 start、finish、abort、续关、体力、门票、奖励和进度已有主流程，剩余分类覆盖仍需审计 | `src/routes/api/singleBattleQuest.ts`、`src/routes/api/storyQuest.ts`、`src/lib/quest/` | [普通剧情结算](../systems/story-quest-settlement.md)、[关卡入场道具](../systems/quest-entry-items.md)、[体力](../systems/stamina.md) |
