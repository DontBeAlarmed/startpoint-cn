...
| `/history/practice_battle` | 固定返回空履历 | 中；练习战主流程可运行，但履历页缺少记录生产者和查询模型 |
...
1. 练习战履历：先从 CN 客户端响应结构和现有战斗结算事实确定最小持久模型。
2. 普通剧情与普通关卡结算：继续检查发奖、进度、任务事实和响应是否处于同一事务。
3. 第二玛纳板时间窗：复核持有状态与客户端可见条件不一致时的 F1009 风险。
4. 支付边界：由项目职责决定完全关闭还是保留本地模拟，不在路由层继续堆叠兼容分支。
