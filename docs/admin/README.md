...
账号页同时展示账号的设备绑定。账号总览使用一次轻量玩家摘要查询并在内存中按账号分组，不随账号或存档数量产生逐账号、逐存档查询。设备名称只用于管理员识别本地设备，空名称表示清除备注，不改变 `device_id -> account_id` 绑定。玩家页的“清除 EX 能力”会同时清空该玩家所有角色的 EX 状态 ID 和能力列表，并返回实际受影响的角色数量；重复执行是成功的零修改操作，不返还任何养成材料。
