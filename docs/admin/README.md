...
当前开放“关卡固定掉落倍率”和“多人结算发放救援碎片”两项设置。掉落倍率允许 `1～10` 的整数，默认值为 `1`；它只影响关卡固定道具、玛纳、经验、属性素材和以太素材的数量，不改变稀有掉落池的命中概率或奖励数量。一次结算只读取一次当前设置，后台保存的新值从之后发生的结算开始生效。

救援碎片开关默认开启。开启后，所有成功多人结算均按关卡档位发放对应银币、金币或紫币碎片 `×10`，不要求房间存在 NPC 或真实救援来源；失败、中止、单人结算和没有碎片映射的关卡不发放。该开关用于真实救援来源尚未补齐时的单人服兼容，后续接入完整救援链路后可由服主关闭。具体映射和推测边界见[多人救援碎片兼容奖励](../systems/multi-rescue-fragments.md)。
