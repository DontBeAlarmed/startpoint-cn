...
| D15 | 全项目领域边界蓝图（识别完成，实施未开始） | [领域边界蓝图](./domain-boundary-blueprint.md) | CDN/客户端/服务端三层边界；owner、有限 shared core、adapter、projector 与独立模块 |
...
D15 只完成领域边界识别、决策和后续实施路线，没有实施 D16 以后重构。阅读 D15 时必须区分 `D15_MAP_STATUS=COMPLETE` 与 `BOUNDARY_MAINLINE_STATUS=NOT_STARTED`，不能把目标 owner/shared core/projector 当成当前运行时事实。
