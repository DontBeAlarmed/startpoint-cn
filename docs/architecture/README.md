...
| D14 | 统一角色成长状态服务目标架构 | [角色成长状态 Gate](./character-growth-state-gate.md) | 同一 Gate 迁移 Growth commands、事务、任务事实、投影、`/load`、存档与性能合同 |
...
D12 与 D13 已完成服务端实现与自动化 Gate，等待客户端实机验收。D14 是尚待实施和验收的目标架构规格，不属于上述当前架构阅读顺序。
...
- D12 与 D13 描述已落地、待客户端实机验收的目标架构；D14 单独描述下一 Gate 的目标架构，在对应 Gate 完成前不得作为当前实现证据。
