...
Release manifest 记录生成时使用的 `gameCalendarUtcOffsetMinutes`，并把它纳入 Release 身份：配置的日历偏移变化后，既有 Release 不再满足复用判定，normal 同步会自动重建；服务启动时要求 `RuntimeConfig` 与当前 Release 的偏移一致，不一致时启动失败关闭。缺少该字段的历史 manifest 只按 CN 默认值 `480` 读取，且不得在非默认偏移下使用；目标偏移为 `480` 时，这类历史 Release 照常参与复用判定，不会仅因缺少该字段而重建，只有版本、补丁来源或表注册等其他重建原因触发时才会重建。详见[游戏业务日历策略](../architecture/game-calendar-policy.md)。
