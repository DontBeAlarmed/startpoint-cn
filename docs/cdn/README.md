...
当前 Registry 为 `88 CDN + 17 bundled + 4 server`。只有已注册 CDN 转换器的表会动态生成，其中 35 张直接表按一至三层 OrderedMap 结构原样还原，6 张奖励表、5 张玩法表按权威字段映射派生，20 张关卡表与 5 张关卡索引在同一批次生成；其余 `assets/` 仍是版本内置兼容数据。加入新 CDN 并不等于任意业务表都会自动更新；但服务端新增、移除注册表或调整表转换元数据后，下一次 normal sync 会自动生成兼容的新 Release，不要求部署者手动 `--force`。
