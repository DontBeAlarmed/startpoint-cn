...
## 土俑奖励动画 C8601 key=0 ✅ 已修复，待客户端复测 (2026-07-17)
...
## 歼灭者讨伐战最高难度不解锁 ✅ 已修复，待客户端验收 (2026-07-17)

**症状**：完成歼灭者活动前置后，最高难度 `200076009` 仍不显示；重新 load 后更容易复现。

**根因**：客户端要求 `200076002/005/006` 三关都具有房主通关状态。服务端多人结算固定返回 `host_finished=true`，却没有把该字段保存到 quest progress，load 响应也不包含它，导致状态无法持久化且成员/房主无法区分。

**修复**：为 quest progress 增加可选 `host_finished`；多人结算只按服务端房间房主判断，不信任客户端身份上报；状态只允许从 false 提升为 true，并在结算与 load 中返回。旧表首次加列时，Advent 已完成记录按旧响应语义一次性补为房主通关，之后不重复回填。

详见 [歼灭者讨伐战最高难度解锁](../systems/boss-epuration-unlock.md)。

## 土俑累计分奖励无动画且未到账 ✅ 已修复，待客户端验收 (2026-07-17)
