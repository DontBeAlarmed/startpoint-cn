...
**修复**：按国服 `AdventEventQuestValues` 使用列 `61/62/63/75` 重建入口消耗数据。包括歼灭者在内的 5 个 Advent 最高难度关卡恢复各自的 1 张门票和 30 体力消耗；生成器也不再过滤零体力的 `Always` 门票关。正式服采用 start 预扣、成功 finish 确认、abort 返还的语义，预扣返还与未完成关卡恢复将由下一独立模块补齐。
