...
## 狂热激战商店为空 ✅ 已按主数据对齐，原始活动批次待客户端验收 (2026-07-19)

**历史症状**：常驻活动批次 `700011-700017` 的兑换列表为空，曾被误判为服务端缺少商品，并推测应共享 `700001-700007` 的商店和文件夹奖励。

**结论**：常驻活动批次没有活动代币、文件夹代币奖励或 `event_item_shop` 商品，空列表是 CN 主数据规定的正确行为。原始活动批次 `700001-700007` 才有 209 件商品，数量依次为 `33/31/29/29/29/29/29`，服务端协议 `event_type=11`。

**修复**：删除商店和文件夹奖励的 `ID-10` 回退；列表按精确活动 ID 和全局服务器时间过滤。普通购买增加正整数校验、JST 开放期校验及 SQLite 总事务，过期直接购买返回 `result_code=2053`，扣币、完整发奖和累计购买数任一步失败均整体回滚。

**待验收**：把全局服务器时间调整到任一原始活动批次开放期，验证商品列表、购买后代币与奖励库存、库存上限以及重新 load 后累计购买数。常驻活动批次只需低优先级回归确认列表保持为空。
