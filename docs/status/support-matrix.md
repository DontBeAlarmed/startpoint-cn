...
| CDN Content Sync | 阶段 B 进行中：109 张表中 52 张由 CDN 动态转换、53 张派生表继续 bundled、4 张为 server 内容；直接表支持一至三层 OrderedMap 递归还原 | 转换器、Release、Catalog、snapshot 和启动同步覆盖较强；35 张直接表已纳入真实 CDN 深度对比，完整 smoke 仍为手工流程 | 官方 CN 1.4.54 基线可运行；Range、断点续传和不同部署模式待专项复核 | [Content Sync](../cdn/content-sync.md)、[运行支持边界](../cdn/runtime-support.md) |
