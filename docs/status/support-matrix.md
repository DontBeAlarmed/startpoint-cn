...
| CDN Content Sync | 阶段 B 进行中：115 张表中 106 张由 CDN 动态转换、5 张继续 bundled、4 张为 server 内容；直接表 37 张（含剧情入队角色和板二开放期）、奖励域 6 张、Additional Reward 1 张、玩法域 5 张、商店域 10 张、活动扭蛋箱 3 张、玛纳节点 1 张、物品装备 8 张、关卡域 20+5 张已动态化 | 转换器、Release、Catalog、snapshot 和启动同步覆盖较强；真实 1.4.54 CDN smoke 锁定关卡 canonical 摘要、奖励引用、剧情入队角色、板二开放期、物品装备差异与装备 lookup 摘要、玩法表、活动扭蛋箱、Additional Reward 及玛纳节点闭包 | 官方 CN 1.4.54 基线可运行；Range、断点续传和不同部署模式待专项复核 | [Content Sync](../cdn/content-sync.md)、[运行支持边界](../cdn/runtime-support.md) |
...
| 多人联机 | NPC 建房、招募、TCP、开始与结算可用；start/continue/finish 已校验玩家级 active 身份，房主承担体力和门票；超级猫头鹰双场景状态机已实现；真人匹配和进程重启恢复未完成 | 房间生命周期、清理、身份、入场与结算边界、房主状态、双代 SceneReady、Notify 索引和内容入口有专项测试 | 基础 NPC 房主流程已有实际使用；多人入场消耗、超级猫头鹰、重赛、昵称显示和 TCP 中断完整矩阵待验收；真人双客户端流程缺少验收条件 | [多人联机协议](../protocol/multi-battle.md) |
