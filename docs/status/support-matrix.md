...
| 邮件 | 列表、单领、全领、后台定向发送已实现；主要装备、抽卡、商店、养成、任务和关卡响应统一动态返回 `mail_arrived` | 附件规则、通知 helper 和部分后台逻辑有覆盖，缺少全部支持类型与通知链路测试 | 若干附件问题已实测修复；当前 12 种支持附件、各业务响应后的未读刷新和异常回滚待完整验收 | [邮件](../systems/mail.md) |
...
| 战阵 | 本地三队 Raid、独立配队、start、事件级累计击破奖励和 finish 已实现 | 主数据解析、阈值选择、重复结算和状态持久化有专项测试，缺少完整协议端到端测试 | 待客户端重新验收，重点确认奖励数量、动画和页面刷新 | [战阵事件级奖励](../systems/raid-event-overall-rewards.md) |
| 歼灭者讨伐战 | 房主解锁、门票预扣、abort 返还和重启恢复已实现 | 房主/成员身份与门票生命周期有覆盖 | 房主解锁已通过；成员不解锁降为低优先级；门票待验收 | [歼灭者解锁](../systems/boss-epuration-unlock.md) |
| 任务系统 | 部分完成：普通/每日/每周、收集、1054 条称号、805 条严格活动协力规则、156 条活动关卡/物品安全规则、Pass 三分类与等级奖励、觉醒已有；Active Mission 已有动态定义读取、开放/依赖/phase 判定、幂等结算、安全领奖核心、Contents Guide 首任务生产链，以及 38 个 pattern（83 条定义）的 `/load` 校准；其中 15 条回归定义仍需资格回调，其余 13 条定义未接入权威生产者；旧 939 条不安全空 selector 规则已移除，Attention 禁用 | QuestRange、活动关卡、累计物品、累计使用技能、累计获得锻造石、单次最高连击、累计冲刺、领主战累计通关、单人最高分、单人限时通关、挑战副本累计通关、指定角色信赖称号、第二玛纳板节点集合、累计体力、累计登录、章节主线与高难全通、练习关卡 SS、珍品商店购买、指定 Boss 超级难度、Active Mission 核心/领奖事务、Contents Guide 首任务、Main/Ex/WorldStoryEvent 任务完成、任务依赖、角色养成、玛纳节点和商店购买状态已有覆盖；其余 1551 条复杂活动谓词、回归资格、Active Mission 其余事实链与其他称号缺完整协议矩阵 | 觉醒核心时序已通过；严格活动规则、Pass、Active Mission、称号条件和其他未验收分类仍需客户端全流程测试 | [任务完成度审计](../systems/mission-completion-audit.md)、[Active Mission](../systems/active-mission.md)、[任务-关卡映射](../systems/mission-quest-mapping.md)、[修行之道](../systems/pass-card.md) |
