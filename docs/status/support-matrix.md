...
| 任务系统 | 部分完成：普通/每日/每周、收集、507 条称号、805 条严格活动协力规则、40 条活动累计物品规则、Pass 三分类与等级奖励、觉醒已有；Active Mission 已有动态定义读取、开放/依赖/phase 判定、幂等结算、安全领奖核心、Contents Guide 首任务生产链，以及 33 条状态/累计事实的 `/load` 校准；其余 63 条业务事实生产者仍未接入；旧 939 条不安全空 selector 规则已移除，Attention 禁用 | QuestRange、累计物品、指定角色信赖称号、Active Mission 核心/领奖事务、Contents Guide 首任务、Main/Ex/WorldStoryEvent 任务完成、任务依赖、登录/体力累计、角色养成与玛纳节点状态已有覆盖；复杂活动谓词、回归资格、Active Mission 其余事实链与其他称号缺完整协议矩阵 | 觉醒核心时序已通过；严格活动规则、Pass、Active Mission 与其他未验收分类仍需客户端全流程测试 | [任务完成度审计](../systems/mission-completion-audit.md)、[Active Mission](../systems/active-mission.md)、[任务-关卡映射](../systems/mission-quest-mapping.md)、[修行之道](../systems/pass-card.md) |
