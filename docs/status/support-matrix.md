...
| CDN Content Sync | 阶段 B 进行中：109 张表中 91 张由 CDN 动态转换、14 张继续 bundled、4 张为 server 内容；直接表 35 张、奖励域 6 张、玩法域 5 张、活动扭蛋箱 3 张、关卡域 20+5 张已动态化 | 转换器、Release、Catalog、snapshot 和启动同步覆盖较强；真实 1.4.54 CDN smoke 锁定关卡 canonical 摘要、奖励引用、元素、入场、解锁索引、玩法表及活动扭蛋箱闭包 | 官方 CN 1.4.54 基线可运行；Range、断点续传和不同部署模式待专项复核 | [Content Sync](../cdn/content-sync.md)、[运行支持边界](../cdn/runtime-support.md) |
