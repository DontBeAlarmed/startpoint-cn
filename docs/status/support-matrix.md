...
| 邮件 | 列表、单领、全领、后台定向发送已实现；全局 `mail_arrived` 语义尚未统一 | 附件规则和部分后台逻辑有覆盖，缺少全部支持类型与通知链路测试 | 若干附件问题已实测修复；当前 12 种支持附件、未读刷新和异常回滚待完整验收 | [邮件](../systems/mail.md) |
...
| 普通关卡 | 单人 start、finish、abort、体力、门票、奖励和进度已实现；普通 finish 尚未整体事务化 | 入场、abort、active 恢复和部分结算规则有覆盖 | 主线常用流程通过；其他普通关卡分类和失败恢复未形成完整矩阵 | [关卡入场道具](../systems/quest-entry-items.md)、[体力](../systems/stamina.md) |
...
| 任务系统 | 部分完成：分类存储、阶段、觉醒结算和 Active Mission 已有；普通、每日、活动、收集、称号、周常无完整自动结算，Pass 6/7/8 缺失 | 存储和局部计算有覆盖，缺少真实全协议矩阵 | 未完成整体验收 | [任务完成度审计](../systems/mission-completion-audit.md) |
