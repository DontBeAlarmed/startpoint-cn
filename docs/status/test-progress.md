...
- 歼灭者讨伐战：三关房主通关、最高难度即时解锁和重新 load 保持均已通过客户端验收；成员不解锁由回归测试覆盖并降为低优先级客户端回归。最高难度歼灭心核扣除已修复，待客户端验收。
...
| 13 | 降临讨伐 | `advent_event_quest` | 459 | ✅ | 🔧 | 暗机兵掉落已通过；歼灭者房主解锁流程已通过，成员不解锁降为低优先级；最高难度门票扣除待客户端验收 |
