...
- 无限激战（Rush）：进入、配队、结算全流程待测；当前已知配队无法保存。
- 土俑（Carnival）：进入与出战已通过；已修复计分时限单位错误，分数与结算待复测。
- 战阵（Raid）：进入、配队、战斗启动与结算全流程待重测。

## 测试清单

| # | 活动名称 | quest JSON | 关数 | 进入 | 结算 | 备注 |
|---|------|------|:---:|:---:|:---:|------|
| 1 | 土俑（Carnival） | `carnival_event_quest` | 171 | ✅ | 🔧 | 进入与出战通过；`battle_time_limit` 帧数换算已修复，分数与结算待复测 |
| 2 | 无限激战（Rush） | `rush_event_quest` | 110 | ⬜ | ⬜ | 已知问题：配队无法保存；列为下一阶段首要测试目标 |
| 3 | 战阵（Raid） | `raid_event_quest` | 50 | ⬜ | ⬜ | 下一阶段重测多人启动、配队与结算 |
