...
- 狂热激战（Rush）：进入、配队、结算全流程待测；配队分类及全局 `party_id` 冲突已修复。CDN 常驻批次没有代币和商品，当前启用 `eventId - 10` 推测兼容；重测常驻列表、首次通关完整奖励、重复通关防重复、购买和持久化。
...
| 2 | 狂热激战（Rush） | `rush_event_quest` | 110 | ⬜ | ⬜ | 配队分类及推测性商店/奖励回退完成；常驻批次重测列表、首次/重复文件夹通关、购买、累计库存和 load 持久化 |
...
| 13 | 降临讨伐 | `advent_event_quest` | 459 | ✅ | 🔧 | 暗机兵掉落已通过；歼灭者房主解锁流程已通过，成员不解锁降为低优先级；最高难度门票预扣、返还和重启恢复待客户端验收 |
