...
特殊关卡统一按“尚未完成客户端验收”处理，不沿用历史测试记录：
...
- 活动扭蛋箱：`box_gacha/reset` 服务端专项测试已通过；活动 28 箱 5 的 2732 库存恢复、次数更新和立即重试拒绝待客户端验收。
- 歼灭者讨伐战：最高难度房主前置及多存档联机身份漂移已修复，待客户端复测；使用默认存档不是列表首项的账号，按“成员不解锁、三关房主通关后立即解锁、重载后保持”验收。

## 测试清单

| # | 活动名称 | quest JSON | 关数 | 进入 | 结算 | 备注 |
|---|------|------|:---:|:---:|:---:|------|
| 1 | 土俑（Carnival） | `carnival_event_quest` | 171 | ✅ | 🔧 | 分数与配队通过；累计分奖励补发、到账、动画及防重复待复测 |
...
| 13 | 降临讨伐 | `advent_event_quest` | 459 | ✅ | 🔧 | 暗机兵掉落已通过；歼灭者多存档房主身份已统一，三关房主前置、即时解锁及重载持久化待客户端复测 |
