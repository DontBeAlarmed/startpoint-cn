...
- 狂热激战（Rush）：进入、配队、结算全流程待测；配队分类及全局 `party_id` 冲突已修复。常驻活动批次无代币和商品是正确行为；原始活动批次商店已完成服务端对齐，待按全局服务器时间进行客户端验收。
...
| 2 | 狂热激战（Rush） | `rush_event_quest` | 110 | ⬜ | ⬜ | 配队分类及商店服务端修复完成；原始活动批次重测列表、购买、累计库存和 load 持久化，常驻空商店低优先级回归 |
