...
- 无限激战（Rush）：进入、配队、结算全流程待测；配队分类隔离已修复，等待客户端验收。
- 土俑（Carnival）：进入与出战已通过；已修复计分时限单位和挑战后配队回退，分数、结算及配队持久化待复测。
...
|---|------|------|:---:|:---:|:---:|------|
| 1 | 土俑（Carnival） | `carnival_event_quest` | 171 | ✅ | 🔧 | 计分与配队分类修复完成；分数、结算及挑战后配队持久化待复测 |
| 2 | 无限激战（Rush） | `rush_event_quest` | 110 | ⬜ | ⬜ | 配队分类隔离已修复；列为下一阶段首要测试目标 |
