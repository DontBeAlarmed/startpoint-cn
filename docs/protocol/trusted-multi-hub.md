...
节点使用壳或操作者配置的共享随机令牌接入 Hub。令牌只接受 32 至 128 位 URL 安全字符，格式为 `[A-Za-z0-9_-]`；推荐生成 32 字节随机数并编码为 64 位十六进制或 43 位 base64url。服务端只能校验格式和长度，随机性与可信分发由服主负责。`123`、空值、空白字符和超长令牌必须在 host/client 启动配置校验阶段拒绝。

Hub 在内存中为每次节点注册生成随机 `nodeSessionId`。后续控制调用使用节点会话凭据，Hub 重启后全部失效。
...
MULTI_HUB_TOKEN=<same-random-secret>
```

`8004` 只注册最小 Hub 控制接口，不暴露主游戏 API、CDN、存档或管理后台。服务端提供显式的 `npm run multi:token` 命令，使用系统安全随机源生成 32 字节随机数并输出 64 位十六进制令牌。交互式终端随后检查项目 `.env`：不存在 `MULTI_HUB_TOKEN` 时询问“是否写入？[Y/n]”，空输入默认写入；已经存在唯一键时询问“是否替换？[y/N]”，空输入默认保留，并提醒替换会使现有节点会话、admission 和房间失效，所有客机也必须更新令牌。只有最终选择为“是”时才新增或替换该键，其他配置和注释必须保持不变。

非交互环境只输出令牌，不等待输入也不写文件。新建 `.env` 时使用仅当前用户可读写的权限；发现重复的 `MULTI_HUB_TOKEN` 或无法安全保存时必须停止，不得猜测覆盖。写入采用同目录临时文件和原子替换。令牌不得进入数据库或普通服务日志。

令牌与地址不得提交仓库或写入普通日志。首期使用受信网络中的共享令牌，不在服务端内实现 TLS。`8004` 不得直接暴露到公网；跨不可信网络时由部署者使用可信 VPN。

共享令牌是运行配置，不属于存档。服务端负责提供生成与格式校验，但不建立令牌数据库，也不实现令牌导入、导出或分发协议；主机与客机只需配置相同的 `MULTI_HUB_TOKEN`。壳或经过认证的管理工具以后可以复用同一生成函数，并提供遮挡显示、复制和扫码导入，但不得改变服务端契约。更换令牌需要重启 Hub，并使现有节点会话、admission 和房间失效。
...
兼容性日志至少包含房间号、差异字段和双方值。摘要可以完整显示，但不得记录共享令牌、节点会话凭据、设备 ID 或登录 token。游戏客户端只接收对应端点现有的 NotPlayable 或通用失败结果；只有房间实际缺失时才显示房间不存在。
