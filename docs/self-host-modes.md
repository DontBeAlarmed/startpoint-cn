...
修法:**zip 内容不含版本号,文件名就是版本边**——把「6 表桥接包 + 7 个角色分包」复制重命名成
从 V 出发的一条边即可(一条边挂多个 zip 协议天然支持),无需重新打包任何数据:
...
服务端会自动感知目录变化(重启更稳妥)。验证:`get_path` 带 `res_ver: V` 应返回 V→W 一跳 8 个 zip。
玩家杀进程重启游戏,下载约 30MB 后商店/角色数据到位。
⚠ 桥接包是**整表替换**(深渊 6 表 + 武器图),角色分包是完整 payload——对任何血统都成立;
但此后你的玩家停在 W 号位,**日后上游再发新内容链(从 1.4.103 起)时需按同样办法再重标一次**。
