...
修法:**zip 内容不含版本号,文件名就是版本边**——把「6 表桥接包 + 7 个角色分包 + 模式资产补充包」
复制重命名成从 V 出发的一条边即可(一条边挂多个 zip 协议天然支持,三组内容路径互不重叠),
无需重新打包任何数据:
...
cp ../archive/pinball-1.4.90-1.4.101-9-modassets07170010.zip "pinball-$V-$W-9-modassets07170010.zip"
```

服务端会自动感知目录变化(重启更稳妥)。验证:`get_path` 带 `res_ver: V` 应返回 V→W 一跳 9 个 zip。
玩家杀进程重启游戏,下载约 30MB 后商店/角色/模式全部到位。

- **补充包是什么**:桥接包只有 6 张表;模式的 31 个客户端资产(武器/横幅/商店图 + 深渊战斗
  floor 表)只在 1.4.90→1.4.101 增量链里。缺它 = 领角色正常,但活动/领主战页面「数据不足」、
  新模式进不去。`archive/pinball-…-modassets….zip` 就是从链尾抽出的这 31 个文件(纯资产+floor,
  与桥接/角色包零路径冲突,任何顺序安全)。
- **已经按旧配方(8 个 zip)修过的服**:玩家已停在 W 号位,只需把补充包再单独发一跳:
  `cp ../archive/pinball-1.4.90-1.4.101-9-modassets07170010.zip "pinball-$W-<W+1>-1-modassets07170010.zip"`。
- ⚠ 桥接包是**整表替换**,角色分包/补充包是完整 payload——对任何血统都成立;但此后你的玩家
  停在自己的号位,**日后上游再发新内容链(从 1.4.103 起)时需按同样办法再重标一次**。
- 修完若仍有「数据不足」:复现一次,把服务端 `logs/http404.log` 里新增的 404 路径发给上游定位。
