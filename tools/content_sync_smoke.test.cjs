...
test("卡池比较忽略费用，但锁定原始行、campaign、类型、ID、数量和 weight", () => {
    ...
    assert.deepEqual(smoke.validateGachas({
        ...
        bundledCampaignDefinitions: campaigns,
        releaseCampaignDefinitions: structuredClone(campaigns),
        bundledRuntime: baseline,
        releaseRuntime: release,
        releaseFeature: feature,
        expectedGachaCount: 2,
        expectedCampaignDefinitions: 1,
        expectedFeature: {
            outer: 1,
            nonemptyOuter: 1,
            entries: 1,
    ...
    assert.throws(() => smoke.validateGachas({
        ...
        releaseRaw: raw,
        bundledCampaignDefinitions: campaigns,
        releaseCampaignDefinitions: campaigns,
        bundledRuntime: baseline,
        releaseRuntime: release,
        releaseFeature: feature,
        expectedGachaCount: 2,
