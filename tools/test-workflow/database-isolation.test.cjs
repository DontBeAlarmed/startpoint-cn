...
try {
    ...
    assert.deepEqual(data.getDatabaseStatus(), { open: true, ready: true, schema: 12 })
    assert.equal(fs.existsSync(path.join(dataDirectory, "wdfp_data.db")), true)
    assert.equal(fs.readFileSync(path.join(dataDirectory, "wdfp_data.db.version"), "utf8"), "12")
