...
function collectedItemTotal(fixture, playerId, itemId) {
    return fixture.db.prepare(`
        SELECT total_obtained
        FROM players_collected_items
        WHERE player_id = ? AND item_id = ?
    `).get(playerId, itemId)?.total_obtained ?? 0
}
...
test("stack_to_exp preserves protection, moves EXP/Star Grain, and returns absolute stack", () => {
    ...
    try {
        ...
        assert.equal(collectedItemTotal(fixture, playerId, 990008), 4)
...
test("bulk_stack_to_exp reads and writes all eligible characters as one growth operation", () => {
    ...
    try {
        ...
        assert.equal(collectedItemTotal(fixture, playerId, 990008), 6)
    } finally {
        fixture.cleanup()
    }
})
