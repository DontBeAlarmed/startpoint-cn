...
def load_names(mod_dir: Path) -> dict[str, str]:
    ...
    # HarvestedPaths.csv:存储位置带 store 前缀(upload/xx/yyy),剥掉前缀匹配
    hp = mod_dir / "HarvestedPaths.csv"
    if hp.exists():
        with hp.open(encoding="utf-8-sig") as fh:
            for row in csv.DictReader(fh):
                name = row.get("逻辑路径")
                loc = row.get("存储位置(含store)") or row.get("存储位置")
                if not (name and loc):
                    continue
                parts = loc.split("/")
                # 形如 upload/xx/yyy -> xx/yyy
                key = "/".join(parts[-2:]) if len(parts) >= 2 else loc
                loc2name.setdefault(key, name)
    return loc2name
