...
test("dashboard renders active patch versions and content release facts", () => {
    ...
    assert.match(source, /status\.cdn\.contentRelease\.contentDigest/)
    assert.match(source, /status\.cdn\.contentRelease\.multiBattleContentDigest/)
    assert.match(source, /Snapshot 声明归档/)
    assert.match(source, /Snapshot 中已声明补丁/)
    assert.match(source, /当前固定 Content Snapshot 未包含补丁/)
    assert.match(source, /gameCalendar: \{\s*\n\s*configuredUtcOffsetMinutes: number\s*\n\s*contentUtcOffsetMinutes: number\s*\n\s*\}/)
    assert.match(source, /status\.cdn\.gameCalendar\.configuredUtcOffsetMinutes/)
    assert.match(source, /status\.cdn\.gameCalendar\.contentUtcOffsetMinutes/)
    assert.match(source, /配置与内容 Release 偏移不一致/)
    assert.doesNotMatch(source, /gameCalendar.*mutation|setGameCalendar|gameCalendar.*api(Post|Put|Delete)/i)
