...
>
> ⚠️ **2026-07-05 起,词条列号/枚举一律以 `mod-tools/词条条件代码全表.md`(+机器可读 `ability_enum_map.json`)为准**——
> 它按客户端解析代码 `AbilityValues.as` 等重建了真实列图(126列,含 schema 漏记的 c3/c4 觉醒列),修正了本手册第四章
> (schema 从 col3 起整体 -1)和多处 off-by-one;并覆盖 leader(-2)/ability_soul(-3)/equipment_enhancement(±0)/ex(-1) 五种布局、
> 全部 209 前置 + 262 触发 + 230 持续触发 + 724 瞬发效果 + 422 持续效果枚举、以及主位/共鸣/目标/固有状态的修改配方。
...
> ⚠️ **本章列号是 AMF3 schema 视角,已知从 col3 起比真实 CSV 少 1 列**(schema 漏记 c3=awake_kind/c4=awake_level 之一)。
> 改数据请改用 `mod-tools/词条条件代码全表.md` 的 126 列真实列图;本章仅留作 schema 对照。
...
#### 对敌伤害效果(瞬发效果列,2026-07-13 反编译 InstantAbilitySource/AbilityDamageShot 实证)

- **kind 33/34/388(DirectDamage/SkillDamage/AbilityDamage)是"XX伤害提升"增伤 buff,不是造成伤害**——
  名字最坑,勿当伤害用。
- 真·对敌造伤三族(均为能力伤害,依攻击族强度 千=1%,70倍=7000000):
  - **251-286 EnemyDamageBy{Attack|CurrentHp|MaxHp}{六元素}[WithoutIncrementCombo]**:
    `time` 列(块内 +22)**空 = AllEnemyDamage 对全体敌人**;填 N = 最近顺序 N 段(NearestOrder)。
  - **316-351 TriggerEnemyDamage…**:打触发源敌人(配受击/敌方行动类触发)。
  - **352-387 NearestEnemyDamage…**:打最近的敌人;`time`=N 为 N 段。
  - 元素在枚举名里(Red..Black 下标 0-5 对应 c3),基准=`target` 列(0=自身攻击/HP);
    伤害经 AbilityDamageShot→NormalAttack impact,`createdByAbility=true` **硬编码**——
    吃"能力伤害提升"buff,**词条层无法造出计为技能伤害的直接伤害**。
- 要"计为技能伤害"用 **kind 629 InvokeSkill**(`string_id` 块内+23、`action_path` 块内+24):
  发动 ability_skill DSL,ActionKind=AbilitySkill → `createdByMainSkillAction=true`=真·技能伤害。
  官方范本:leader_ability 111183 行5(火龙弹射追击);"打最近敌人"DSL 范本=
  `rare4/marching_rival$marching_rival_1`(FindNearSubjects→CreateHitArea→CreateNormalAttack)。
  新 DSL 文件=新逻辑路径写 store+发布(官方未下发=新建,客户端照常下载)。
  ⚠ **string_id 必须注册进 `master/string/custom_ability_string.orderedmap`**(单列=词条描述
  原文文案):角色页描述生成走 `CustomAbilityStringTable.get(string_id)`,缺键 →
  **C8601「指定的Key不存在」**(界面误导性显示"部分资源文件已损坏",2026-07-13 实锤)。
  GUI composer_apply 已内置缺键自动注册(默认文案「发动特殊技能」);发布别名
  `custom_ability_string`。
- **空白行哨兵**:枚举列空串=C7050(见 §十二安全规则);非 Initial 触发的
  threshold/上限/冷却 官方惯例=0/0、(None)、0(空串=Some(null) 行为未定义,少量官方空串
  存在但别效仿)。
...
| 4 | race | 种族,逗号分隔多值:Human人型 Beast兽型 Element精灵 Machine机械 Undead不死 Mystery神秘 Dragon龙族 Devil魔族 Plants植物 Aquatic水栖。**详情页种族显示读这里**(token 查 APK 内 race 表取名称/图标;store 侧 race 表是 16B 空壳,未知 token 显示不出来) |
| 5 | character_tag | 标签数组(逗号分隔),OmniElement 共鸣通用标签挂这里 |
| 6 | speciality_type | 强化弹射种类:`0`剑士 `1`格斗 `2`射击 `3`辅助 `4`特殊(兼类型图标) |
| 7 | gender | `Male` / `Female` / 空 |
| 8 | action_skill | 主动技标识(常同 code_name),= action_skill 表键 |
| 9–16 | 形态切换 | col9=切换条件 kind(`(None)`/0-4) col10-16=条件参数/目标技能等(见 SKILL.md 形态切换区) |
| 17 | leader_ability_id | **队长技 = leader_ability 表键**(如白虎=10 此列为 `3` → L:3;`(None)`=无队长技) |
| 18 | leader_ability_name | 队长技称号(中文)。**详情页队长技名读这里,不读 character_text[10]**(CharacterValues 把 c17+c18 组成 leader_ability Option;进化后客户端套 `XXX+/++` 模板) |
...
> ⚠ **显示源铁律(2026-07-13 逆向 StatusWindow/CharacterTextValues)**:本表(=②层
> `character_text`,客户端 CharacterTextValues 同布局,[8]/[9]=技能名/描述＋＋)的
> 技能名/技能描述(4-9)与队长技称号(10)**只在抽卡特性页显示**(GachaFeaturePageHolderView)。
> 游戏**详情页/战斗**读的是:技能名/描述 = `action_skill` 表 name/description(按技能级别),
> 队长技名 = `character` 表 c18。只改本表这三类字段游戏里看不到变化——GUI 资料页保存时
> 已自动三处同步(character_text + action_skill + character c18),手写脚本要自己同步。
...
| 表 | 说明 |
|---|---|
| `leader_ability` | 键=角色 ID;**没有 unisonable 列**(其 c1 是 awake_kind,旧记载"0/1"有误),块基址=ability-2;移植到 ability 表时 c1 位置必须写 `true`/`false` |
| `ability_soul`(436 键) | =武器(equipment)本体词条;块结构同 ability 但**基址-3**,头部为 slot/learn_level/trigger(详见词条条件代码全表 §2) |
